...
class OTPViewController: BaseViewController {
    ...
    @IBOutlet weak var artitleLabel: BaseLabel!{
        didSet {
            artitleLabel.text = "أدخل رقم التعريف الشخصي الذي تلقيته عبر الرسالة النصية"
            artitleLabel.textAlignment = .center
        }
    }
    ...
    @IBOutlet weak var verifyButton: RoundedGreenButton?
...
//                return
    ...
    func setText(){
        if LanguageHelper.getLanguage() == .arabic {
            self.verifyButton?.setTitle("إرسال", for: .normal)
        }else {
